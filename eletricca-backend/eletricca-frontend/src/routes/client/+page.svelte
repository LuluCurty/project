<script>
	import { Plus, FileUp, Search, Trash, Ellipsis, CircleX } from '@lucide/svelte';
	...
	async function getClients() {
		try {
			const res = await fetch(`/api/client?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}`, {
				credentials: 'include'
			});
